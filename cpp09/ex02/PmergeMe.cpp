...
void	PmergeMe::printTime(std::vector<int>& container){
	std::cout << "Time to process a range of " << container.size() << " elements with std::vector<int> : " << std::fixed << std::setprecision(6) << vectorTime << " s" << std::endl;
}

void	PmergeMe::printTime(std::list<int>& container){
	std::cout << "Time to process a range of " << container.size() << " elements with std::list<int> : " << std::fixed << std::setprecision(6) << listTime << " s" << std::endl;
}
...
bool	PmergeMe::checkNums(int ac, char **av){
	for (int i = 1; i < ac; i++)
