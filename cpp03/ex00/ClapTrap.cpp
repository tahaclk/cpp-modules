...
void ClapTrap::takeDamage(unsigned int amount){
    ...
    {
        hitPoints -= amount;
        std::cout << name << " took " << amount << " damage. (" << hitPoints << " hit points left)" << std::endl;
    }
    else if (hitPoints > 0)
    {
        hitPoints = 0;
        std::cout << name << " took " << amount << " damage and dead. (" << hitPoints << " hit points left)" << std::endl;
    }
    else
    {
        std::cout << name << " is already dead. Why do you keep hitting?" << std::endl;
    }
}
